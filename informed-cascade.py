...
if __name__ == '__main__':
    ...
    # Test all predictors
    fnames = []
    basename = 'I-waterfall-conf'+str(db_config)
    for i in range(db_config):
    ...
    for m in methods_fusion:
        ...
        for f, r in zip(features, labels_test):
            ...
            if p1!='TW':
                det = 'D2-'+p1
                p = predictors[det+'-'+m].predict([f])[0]
                p2 = maps[det][p]               
        ...
        for i in range(db_config):
            save_waterfall(len(merged_classes[i]), cm[i], merged_classes[i], fnames[i], m)
